$(document).ready(function(){
    ...
    const startWebSocket = function(){
        ...
        ws.onmessage = (function(ev) {  
            ...
            if(obj.status ==="connectionNotification") {
                ...
                $('.chatItems').append(html)
                ws.close()
                $('button#sendMessage').addClass('hidden')
                $('button#nextChat').removeClass('hidden')
    ...
    $('#nextChat').on('click', function(e){
        startWebSocket()
        $('textarea').attr('disabled', false)
        $('.chatItems').html('')
        $('button#nextChat').addClass('hidden')
        $('button#sendMessage').removeClass('hidden')        
    })
